...
export class ShoppingCart{
    ...
    addToCart(product:Product,selectedSize:string){
        let cartItem = this._cartItems.find(cartItems =>
             cartItems.product.id == product.id);
    ...
    subFromCart(product:Product){
        let cartItem = this._cartItems.find(cartI =>
            cartI.product.id == cartItem.product.id
        );
        if(cartItem){
            cartItem.quantity -= 1;
    ...
    removeCartItem(cartItem:CartItems){
        let index = this._cartItems.findIndex(cartI =>
            cartI.product.id == cartI.product.id
            );
    ...
    get total(){
        return this._cartItems.reduce((acc,cartI) =>
            acc + (cartI.quantity * cartI.product.price),0)
    }
